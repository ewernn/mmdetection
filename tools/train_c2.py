...
class AdjustBrightness:
    ...
    def __call__(self, image, target):
        brightness_factor = random.uniform(self.brightness_factor[0], self.brightness_factor[1])
...
class AdjustContrast:
    ...
    def __call__(self, image, target):
        contrast_factor = random.uniform(self.contrast_factor[0], self.contrast_factor[1])
...
def get_transform(train):
    ...
    if train:
        transforms.extend([
            ...
            AdjustBrightness((0.3, 1.5)),
            AdjustContrast((0.3, 1.5)),
...
def evaluate(model, data_loader, device, epoch):
    ...
    for i, (images, targets) in enumerate(data_loader):
        ...
        for j, (target, output) in enumerate(zip(targets, outputs)):
...
def create_model(args, num_classes):
    ...
    anchor_sizes = (
        ...
        (400, 450),   # Medium objects
        (525, 525),   # Medium-large objects
        (700, 615),   # Large objects
    )
...
def parse_arguments():
    ...
    parser.add_argument('--roi_heads_score_thresh', type=float, default=0.6, help='ROI heads score threshold')
    parser.add_argument('--rpn_bg_iou_thresh', type=float, default=0.5, help='RPN background IoU threshold')
    parser.add_argument('--rpn_fg_iou_thresh', type=float, default=0.7, help='RPN foreground IoU threshold')
    parser.add_argument('--roi_heads_batch_size_per_image', type=int, default=64, help='ROI heads batch size per image')
    ...
    parser.add_argument('--rpn_pre_nms_top_n', type=int, default=300, help='RPN pre-NMS top N')
    parser.add_argument('--rpn_post_nms_top_n', type=int, default=100, help='RPN post-NMS top N')
